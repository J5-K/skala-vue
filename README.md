...
- Element Plus와 기본 스타일을 애플리케이션에 전역 등록했습니다.
- 검색과 온도 입력에 `el-input`, `el-input-number`를 사용했습니다.
- 날씨 카드와 이동 가이드는 `el-card`, 상태 배지는 `el-tag`로 개선했습니다.
- API 로딩·오류·빈 검색 결과를 `el-skeleton`, `el-alert`, `el-empty`로 구분했습니다.
- 상세 기상 정보는 `el-descriptions`를 이용해 항목별로 정리했습니다.
- 직접 만든 `BaseDashboardCard`와 레이아웃은 유지하고 공통 UI 요소에만 Library를 적용했습니다.
