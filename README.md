...
| Day 3 | Vue Router, Pinia, Axios 실시간 날씨           | 완료 |

---

> Day 1·2 내용은 당시 단계별로 구현했던 기능을 기록한 것으로, 일부 기능은 이후 실시간 날씨 서비스에 맞게 변경하거나 제거했습니다. Day별 항목에는 최종적으로 구현한 기능과 적용한 기술을 중심으로 정리했고, 구현 과정에서 발생한 오류와 원인·해결 과정은 문서 하단의 **트러블슈팅**에 따로 모았습니다.
...
- 상세 페이지에 진입한 시점의 최신 API 결과를 표시합니다.
...
- 이동 가이드는 생활정보 화면에 배치하고, 이미 불러온 선택 지역 날씨를 props로 전달해 생활 수칙과 함께 확인할 수 있게 했습니다.
...
사용자가 입력한 기준이 새로고침할 때마다 사라지지 않도록 추위·더위 기준을 `configStore`로 옮기고 `localStorage`에 저장했습니다. 날씨 판단에는 API에서 받은 섭씨 원본을 사용하고, 카드 배지에 보이는 기준값만 `convertTemperature()`로 변환해 현재 단위와 일치하도록 정리했습니다.
...
- 선택한 도시 ID를 Pinia Store에 저장하고, 휴식 화면에서는 해당 도시의 최신 날씨를 다시 요청해 사용했습니다.
...
고정 너비를 `width: 100%`와 `max-width` 기반으로 변경하고 `margin: 0 auto`로 가운데 정렬했습니다. 홈 화면은 CSS Grid로 검색·온도 기준 영역과 날씨 목록을 2열로 배치하고, 화면이 좁아지면 미디어 쿼리로 1열이 되도록 구성했습니다. 상세 날씨·여행지 검색·휴식 도우미에도 같은 너비 기준을 적용했습니다.
...
### 16. 실시간 날씨에 맞게 카드 기능 정리

Day 1·2에는 `v-model`, 이벤트 처리와 반응형 변화를 직접 확인하기 위해 카드마다 온도를 낮추거나 높이고 숫자를 직접 입력하는 기능을 만들었습니다.

하지만 OpenWeather API로 실제 날씨를 받아오기 시작한 뒤 다시 생각해 보니, 현재 기온을 사용자가 임의로 바꾸는 기능은 실시간 날씨 서비스와 어울리지 않았습니다. 섭씨에서 화씨로 단위를 변경했을 때 입력창과 `- / +` 버튼의 기준도 달라져 사용자에게 혼란을 줄 수 있었습니다.

그래서 최종 화면에서는 수동 온도 조절 기능과 관련 이벤트를 제거하고, API에서 받은 기온은 그대로 보여주도록 정리했습니다. 대신 비어 있는 공간을 활용해 판교·울산·광주 이름과 현재 기온을 더 크게 표시하고, 날씨 상태는 별도 태그로 구분했습니다. 체감온도·습도·풍속도 한눈에 읽을 수 있도록 간격과 글자 크기를 조정했습니다.

이번 작업을 통해 처음 배운 기능을 모두 남겨두는 것보다, 실제 데이터를 사용하기 시작한 시점에는 서비스 목적에 맞는 기능인지 다시 판단하고 정리하는 과정도 필요하다는 점을 느꼈습니다.

### 17. 국내·해외 여행지 날씨 검색 개선

기존 도시 검색 기능을 서비스의 휴식 주제와 연결하기 위해, 단순한 날씨 검색을 여행지를 고를 때 활용하는 기능으로 바꾸었습니다. 사용자가 검색 범위를 쉽게 구분할 수 있도록 국내와 해외 선택 기능도 추가했습니다.

- `searchMode`를 `ref()`로 관리하고 국내·해외 버튼을 누르면 검색 조건이 바뀌도록 했습니다.
- 국내 검색에서는 도시 이름에 국가 코드 `KR`을 함께 전달하고, 한국에 해당하는 결과만 표시했습니다.
- 해외 검색에서는 한국을 제외한 도시 후보를 보여주도록 결과를 구분했습니다.
- 검색 모드와 검색어를 URL query에 함께 저장해 새로고침 후에도 검색 상태가 유지되도록 했습니다.
- 검색 범위를 변경하면 이전 검색 결과와 선택한 날씨를 초기화해 서로 다른 범위의 결과가 섞이지 않도록 했습니다.
- 모바일에서는 국내·해외 선택 버튼과 검색창이 세로로 배치되도록 반응형 스타일을 적용했습니다.

처음에는 OpenWeather API를 고정된 지역의 현재 날씨를 보여주는 용도로만 사용했습니다. 이번에는 같은 API를 여행지 검색과 날씨 확인이라는 다른 역할로 연결해 보면서, 새로운 API를 계속 추가하지 않아도 기존 응답과 검색 조건을 어떻게 활용하느냐에 따라 서비스 기능을 확장할 수 있다는 점을 느꼈습니다.

다만 OpenWeather Geocoding API는 일부 글자나 오타를 이용한 검색을 안정적으로 지원하지 않아, 도시 이름 전체를 입력하는 방식을 기준으로 구현했습니다.

### 18. 선택 지역 새로고침 유지

내 지역으로 설정한 캠퍼스가 새로고침하면 초기화되는 불편함이 있어, 선택한 도시 ID를 `localStorage`에 저장하도록 개선했습니다.

- Pinia의 `selectedCityId` 초기값을 `localStorage`에서 불러오도록 했습니다.
- 지역을 선택하면 Pinia 상태와 `localStorage`의 도시 ID가 함께 변경됩니다.
- 판교·울산·광주의 ID와 좌표는 `cityTargets.js`로 분리해 홈과 휴식 화면에서 함께 사용했습니다.
- 휴식 도우미를 새로고침하면 저장된 ID로 선택 지역을 찾고, 해당 도시의 최신 날씨를 다시 요청합니다.

지역이 세 곳뿐이기 때문에 저장 공간을 줄이는 것이 주된 목적은 아니었습니다. 날씨 객체 전체를 저장하면 이전 기온이 남을 수 있다고 생각해, 선택 상태인 도시 ID만 저장하고 실시간 날씨는 필요할 때 API에서 다시 가져오도록 역할을 나누었습니다.

이를 통해 새로고침 후에도 내 지역 선택은 유지하면서, 이동 가이드와 휴식 추천에는 오래된 저장값이 아닌 최신 날씨를 사용할 수 있게 되었습니다.

### 19. 선택 지역에 맞는 날씨 생활정보

기존 생활정보 화면에는 맑음·비·구름에 대한 간단한 안내만 있어서, 실제로 지금 어떤 내용을 확인해야 하는지 바로 알기 어려웠습니다. 이미 내 지역의 실시간 날씨를 받아오고 있었기 때문에, 이 데이터를 생활정보에도 활용해 보기로 했습니다.

- 선택한 지역의 최신 날씨를 OpenWeather API로 불러왔습니다.
- 현재 기온과 날씨 상태, 풍속을 기준으로 지금 필요한 생활정보를 먼저 보여주도록 했습니다.
- 더위·추위·비·눈·강풍·맑음·흐림 상황에 따라 행동 요령과 준비사항을 다르게 구성했습니다.
- 전체 생활정보는 `el-collapse`로 정리해 필요한 내용만 펼쳐볼 수 있게 했습니다.
- 현재 날씨와 관련된 항목은 자동으로 열리도록 연결했습니다.
- 지역을 설정하지 않았거나 API 요청 중·실패 상황일 때도 각각 안내 화면이 나오도록 처리했습니다.

처음에는 모든 정보를 카드로 나열하려고 했지만, 항목이 늘어나면 오히려 원하는 내용을 찾기 어렵다고 생각했습니다. 그래서 현재 필요한 정보는 위에서 바로 보여주고, 나머지는 접어서 확인하는 방식으로 정리했습니다. 같은 날씨 데이터라도 이동이나 휴식뿐 아니라 생활 준비를 돕는 정보로도 활용할 수 있다는 점을 확인했습니다.

### 20. 반복되는 날씨 요청 코드 정리

휴식 도우미와 생활정보 화면에서 함께 사용할 수 있도록 선택 지역의 최신 날씨 요청 과정을 `useSelectedCityWeather` composable로 분리했습니다.

- Pinia에서 선택 지역 ID 확인
- `cityTargets`에서 해당 도시와 좌표 찾기
- Axios로 OpenWeather API 요청
- API 응답을 공통 날씨 객체로 변환
- 로딩 상태와 오류 메시지 관리
- 선택 지역 변경 시 최신 날씨 다시 요청

공통화한 뒤 휴식 도우미에는 휴식 활동을 추천하는 코드만, 생활정보 화면에는 날씨별 행동 요령을 판단하는 코드만 남길 수 있었습니다.

### 21. 서비스 소개와 내비게이션 UI 개선

기능이 늘어나면서 처음 방문한 사람이 이 서비스가 어떤 과제인지 한눈에 이해하기 어렵다고 느꼈습니다. 기존 서비스 소개 화면도 Router 실습 내용을 간단히 보여주는 수준이어서, 현재 구현된 기능과 3일 동안의 진행 사항을 중심으로 다시 구성했습니다.

- `el-card`로 실시간 날씨·이동과 생활 가이드·잠깐 쉬어가기·여행지 날씨 기능을 나누고, 각 화면으로 바로 이동하는 버튼을 추가했습니다.
- `el-steps`로 Day 1부터 Day 3까지 기능을 발전시킨 과정을 순서대로 표시했습니다.
- `el-descriptions`로 Vue Router, Pinia, Axios 등 배운 기술을 실제로 어디에 사용했는지 정리했습니다.
- README에 기록한 대표적인 트러블슈팅은 `el-collapse`로 구성해 필요한 내용만 펼쳐볼 수 있게 했습니다.
- 서비스 소개 상단에는 REST API의 `REST`와 잠깐 쉬어가는 `rest`를 함께 담은 서비스 이름의 의미를 적었습니다.

처음에는 영역을 구분하기 위해 영문 소제목을 사용했지만, 과제 설명과 잘 어울리지 않고 다소 꾸며진 느낌이 강했습니다. 그래서 `구현한 기능들`, `3일 동안 진행 사항`, `기술 사용 내역`, `문제를 해결하며 배운 점`처럼 실제 내용을 바로 알 수 있는 한국어 제목으로 바꾸고, 작은 아이콘과 왼쪽 강조선을 이용해 구역이 눈에 들어오도록 정리했습니다.

내비게이션도 사용 순서를 생각해 다음과 같이 변경했습니다.

```text
오늘의 날씨 → 생활정보 → 쉬어가기 → 여행지 날씨 → 서비스 소개
```
...
| 국내·해외 검색 구분   | `ref()`, 국가 코드, `filter()`                     |
| 검색 모드 URL 유지    | `route.query`, `router.push()`                     |
| 선택 지역 영구 유지   | Pinia, `localStorage`                              |
| 최신 선택 지역 복원   | `watch()`, 도시 ID, Axios                          |
| 맞춤 생활정보 계산    | `computed()`, 조건문                               |
| 상황별 정보 접기      | `el-collapse`, `v-for`                             |
| 날씨 요청 로직 재사용 | Composable                                         |
| 선택 지역 변경 감지   | `watch()`, Pinia                                   |
...
- 실시간 API 원본 기온과 확대된 날씨 카드 정보 표시
- 기준 온도에 따른 추움·적당함·더움 배지 자동 변경
- 카드 선택 상태 표시와 도시별 상세 화면 이동
- 새로고침 후 검색어 복원
- 상세 페이지 진입 시 선택 도시의 최신 날씨와 예보 조회
- 날씨 생활 정보 및 404 화면
...
실습 전용 `PracticeView.vue`를 만들고 `/practice` 경로에 연결했습니다.
...
### 6. 선택 지역 날씨 요청 코드가 반복되던 문제

생활정보 기능을 추가하고 나니 휴식 도우미와 생활정보 화면에서 선택 지역을 찾고 날씨 API를 요청하는 과정이 반복되고 있었습니다.

처음에는 화면마다 코드를 따로 작성하는 편이 이해하기 쉬웠지만, API 주소나 응답 처리 방식이 바뀐면 여러 파일을 함께 수정해야 했습니다. 반복되는 과정을 `useSelectedCityWeather` composable로 분리하고, 각 화면에는 휴식 추천과 생활정보를 계산하는 코드만 남겼습니다.

이 작업으로 API 요청 횟수나 실행 속도가 달라진 것은 아니지만, 공통 코드를 한곳에서 관리할 수 있게 되었고 각 화면의 역할도 이전보다 명확해졌습니다.

### 7. 상세 페이지에 이전 온도가 표시되던 문제

상세 페이지에서 최신 날씨 API를 다시 요청하고 있었지만, 화면에는 홈에서 URL query로 넘겨준 이전 기온과 날씨 상태가 우선적으로 표시되고 있었습니다.

표시할 데이터가 query 값을 먼저 사용하도록 작성된 것이 원인이었습니다. query의 온도·상태 정보에 의존하던 코드를 제거하고, route의 도시 ID로 `cityTargets`에서 좌표를 찾은 뒤 상세 화면에서 받은 최신 API 결과를 사용하도록 수정했습니다.

이 과정을 통해 API를 호출했는지만 확인하는 것이 아니라, 최종적으로 화면에 표시되는 값의 우선순위까지 확인해야 한다는 점을 알게 됐습니다.

### 8. 화면마다 온도 단위가 다르게 표시되던 문제

날씨 카드와 상세·5일 예보는 섭씨·화씨 변경을 반영했지만, 홈의 평균 온도와 생활정보의 현재 기온은 항상 섭씨로 표시되고 있었습니다.

일부 화면에서 `°C`를 직접 작성하고 온도 변환 함수를 사용하지 않은 것이 원인이었습니다. 해당 표시 값에도 `configStore`의 `convertTemperature()`와 `unitSymbol`을 적용해 어느 페이지에서든 같은 단위가 나타나도록 맞췄습니다.

날씨를 판단하는 계산에는 API의 섭씨 원본을 그대로 사용하고, 사용자에게 보여주는 값만 변환하면 데이터의 기준을 유지하면서도 표시 단위를 통일할 수 있다는 점을 확인했습니다.

### 9. 추천 여행지의 사진이 반복되던 문제

Pexels API에서 각 도시의 첫 번째 결과만 사용하자 추천 여행지가 바뀌어도 비슷한 사진이 계속 표시되는 느낌이 있었습니다.

도시별로 사진을 3장씩 받고 그중 한 장을 랜덤으로 선택하도록 변경했습니다. 같은 도시가 다시 나왔을 때는 이미 받은 사진 목록을 `Map`에서 불러와 불필요한 API 요청을 줄였고, 사진을 불러오지 못하면 기본 화면을 표시하도록 처리했습니다.

랜덤 결과를 늘리기 위해 API를 매번 다시 호출하기보다, 한 번 받은 결과를 재사용하는 방식이 화면 변화와 API 사용량을 함께 고려하는 데 더 적합하다고 판단했습니다.
