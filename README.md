...
기능이 늘어나면서 처음 방문한 사람이 이 서비스가 어떤 과제인지 한눈에 이해하기 어렵다고 느꼈습니다. 기존 서비스 소개 화면도 Router 실습 내용을 간단히 보여주는 수준이어서, 현재 구현된 기능과 3일 동안의 발전 과정을 중심으로 다시 구성했습니다.

- `el-card`로 실시간 날씨·생활정보·휴식 도우미·여행지 검색 기능을 나누고, 각 화면으로 바로 이동하는 버튼을 추가했습니다.
...
처음에는 영역을 구분하기 위해 영문 소제목을 사용했지만, 과제 설명과 잘 어울리지 않고 다소 꾸며진 느낌이 강했습니다. 그래서 `이런 기능을 담았어요`, `3일 동안 이렇게 발전시켰어요`처럼 실제로 내용을 설명하는 한국어 제목으로 바꾸고, 작은 아이콘과 왼쪽 강조선을 이용해 구역이 눈에 들어오도록 정리했습니다.
