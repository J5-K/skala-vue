...
> Day 1·2 내용은 당시 단계별로 구현했던 기능을 기록한 것으로, 일부 기능은 이후 실시간 날씨 서비스에 맞게 변경하거나 제거했습니다.
...
처음에는 이동 가이드를 홈 화면 아래에 배치했지만, 날씨 카드와 설정 기능이 모인 화면이 길어져 생활정보 화면으로 옮겼습니다. 생활정보에서 이미 불러온 선택 지역 날씨를 props로 전달하므로 같은 데이터를 다시 요청하지 않고, 생활 수칙과 이동 준비를 한곳에서 확인할 수 있습니다.

홈의 평균 온도와 생활정보의 현재 기온도 `configStore`의 단위 설정을 사용하도록 맞췄습니다. 섭씨 원본 데이터는 유지하고 화면에 표시하는 값만 변환해 어느 페이지에서 보더라도 같은 단위가 나타나도록 했습니다.
...
생활정보 기능을 추가하고 나니 휴식 도우미와 생활정보 화면에서 비슷한 코드가 반복되고 있다는 것을 발견했습니다. 두 화면 모두 선택 지역을 찾고, 좌표로 날씨를 요청한 뒤 필요한 형태로 데이터를 바꾸는 과정을 각각 작성하고 있었습니다.

처음에는 화면마다 따로 작성하는 편이 이해하기 쉬웠지만, 같은 코드를 계속 유지하면 API 주소나 응답 형식이 바뀔 때 여러 파일을 함께 수정해야 한다는 문제가 있었습니다. 그래서 공통 부분을 `useSelectedCityWeather` composable로 분리했습니다.
...
이번 작업이 API 요청 횟수를 줄이거나 화면 속도를 크게 높인 것은 아닙니다. 대신 반복되는 코드를 한곳에서 관리하게 되어 수정해야 할 범위가 줄었고, 각 화면이 어떤 역할을 담당하는지도 이전보다 이해하기 쉬워졌습니다.
...
- README에 기록한 트러블슈팅은 `el-collapse`로 구성해 필요한 내용만 펼쳐볼 수 있게 했습니다.
