...
watch(currentCity, (newValue, oldValue) => {
  logMessage.value = `감시자 발동중 [${oldValue}]에서 [${newValue}]로 변경됨`
...
<template>
  <div class="practice-section">
    ...
    <button @click="currentCity = '판교'">판교 선택</button>
    <button @click="currentCity = '울산'">울산 선택</button>
    <button @click="currentCity = '광주'">광주 선택</button>

    <div class="monitor">
      <h3>파수꾼(watch) 모니터링 시스템</h3>
      <p>{{ logMessage }}</p>
      <small style="color: gray">(버튼을 누른 후 브라우저콘솔창 F12를 확인하세요)</small>
    </div>
  </div>
</template>
